...
function scoreSavingsRate(income: number, netCashFlow: number): SubScore {
  ...
  if (rate >= 0.2) {
    return {
      ...
      label: "Savings Rate",
      score: 20,
      status: `Saving ${pct} of income — excellent. Most financial plans suggest 20% as a minimum.`,
      tip: null,
    };
  }
  if (rate >= 0.15) {
    return {
      ...
      label: "Savings Rate",
      score: 16,
      status: `Saving ${pct} of income — nearly there. Target is 20%+.`,
      tip: "Saving a few more percent would reach the recommended minimum.",
    };
  }
  if (rate >= 0.1) {
    return {
      ...
      label: "Savings Rate",
      score: 12,
      status: `Saving ${pct} of income — below the 20% target.`,
      tip: "Try reducing one discretionary expense to push your savings rate higher.",
    };
  }
  if (rate >= 0.05) {
    return {
      ...
      label: "Savings Rate",
      score: 6,
      status: `Saving only ${pct} of income — below the recommended 20%.`,
      tip: "Review your largest spending categories to find areas to cut back.",
    };
  }
  if (rate >= 0) {
    return {
      ...
      label: "Savings Rate",
      score: 2,
      status: "Saving less than 5% of income this period.",
      tip: "Even saving 1–2% more each month compounds significantly over time.",
    };
  }
  return {
    id: "savings_rate",
    label: "Savings Rate",
    score: 0,
    status: "Spending exceeds income this period — negative savings rate.",
    tip: "Review recurring expenses and identify spending that can be reduced.",
  };
}
...
function scoreBudgetAdherence(budgets: Budget[]): SubScore {
  ...
  if (overCount === 0) {
    return {
      ...
      label: "Budget Adherence",
      score: 20,
      status: `All ${budgets.length} budget${budgets.length !== 1 ? "s" : ""} within limit — great discipline.`,
      tip: null,
    };
  }
  if (adherenceRate >= 0.75) {
    return {
      ...
      label: "Budget Adherence",
      score: 15,
      status: `${within} of ${budgets.length} budgets within limit. ${overCount} ${overCount === 1 ? "is" : "are"} over.`,
      tip: "Focus on the over-budget categories to improve this score.",
    };
  }
  if (adherenceRate >= 0.5) {
    return {
      ...
      label: "Budget Adherence",
      score: 10,
      status: `${within} of ${budgets.length} budgets within limit. Multiple categories are over.`,
      tip: "Consider adjusting limits to realistic targets or reducing spending.",
    };
  }
  if (adherenceRate >= 0.25) {
    return {
      ...
      label: "Budget Adherence",
      score: 5,
      status: `Most budgets are over limit — ${overCount} of ${budgets.length} exceeded.`,
      tip: "Start with one category and commit to staying within its limit for a month.",
    };
  }
  return {
    id: "budget_adherence",
    label: "Budget Adherence",
    score: 0,
    status: `Nearly all budgets are over limit — ${overCount} of ${budgets.length} exceeded.`,
    tip: "Reset limits to realistic levels and track one category at a time.",
  };
}
...
function scoreEmergencyCoverage(liquidBalance: number, monthlySpending: number): SubScore {
  if (monthlySpending <= 0) {
    return {
      ...
      score: 10,
      status: "No spending recorded this period — score is neutral.",
      tip: "Record spending to see your emergency fund coverage.",
    };
  }
  ...
  if (months >= 6) {
    return {
      ...
      label: "Emergency Coverage",
      score: 20,
      status: `${monthsStr} months of expenses covered — excellent emergency fund.`,
      tip: null,
    };
  }
  if (months >= 3) {
    return {
      ...
      label: "Emergency Coverage",
      score: 15,
      status: `${monthsStr} months of expenses covered. The 3–6 month target is met.`,
      tip: "Building toward 6 months would achieve full marks.",
    };
  }
  if (months >= 2) {
    return {
      ...
      label: "Emergency Coverage",
      score: 10,
      status: `${monthsStr} months of expenses covered. Target is 3–6 months.`,
      tip: "Add to your savings account to build a stronger emergency buffer.",
    };
  }
  if (months >= 1) {
    return {
      ...
      label: "Emergency Coverage",
      score: 6,
      status: `${monthsStr} months of expenses covered — below the recommended minimum.`,
      tip: "A 3-month emergency fund can prevent costly debt during unexpected events.",
    };
  }
  return {
    id: "emergency_coverage",
    label: "Emergency Coverage",
    score: 0,
    status: "Less than 1 month of expenses in liquid savings — no emergency cushion.",
    tip: "Start by saving one month of expenses in a dedicated savings account.",
  };
}
...
function scoreNetWorthTrend(history: NetWorthSnapshot[]): SubScore {
  if (history.length < 2) {
    return {
      ...
      score: 10,
      status: "Not enough history to assess trend — score is neutral.",
      tip: "Continue using Bloom to build up a net worth history.",
    };
  }
  ...
  if (trend > 50 && isPositive) {
    return {
      ...
      label: "Net Worth Trend",
      score: 20,
      status: `Net worth is positive and growing (${trendStr} last month).`,
      tip: null,
    };
  }
  if (trend > 50 && !isPositive) {
    return {
      ...
      label: "Net Worth Trend",
      score: 14,
      status: `Net worth is improving (${trendStr}) but still negative — heading in the right direction.`,
      tip: "Keep the trend going and net worth will turn positive.",
    };
  }
  if (Math.abs(trend) <= 50 && isPositive) {
    return {
      ...
      label: "Net Worth Trend",
      score: 12,
      status: "Net worth is positive and holding steady.",
      tip: "Growing net worth month-over-month would reach full marks.",
    };
  }
  if (Math.abs(trend) <= 50 && !isPositive) {
    return {
      ...
      label: "Net Worth Trend",
      score: 6,
      status: "Net worth is negative and flat — no recent improvement.",
      tip: "Increasing savings or reducing debt will start moving this number up.",
    };
  }
  if (trend < -50 && isPositive) {
    return {
      ...
      label: "Net Worth Trend",
      score: 5,
      status: `Net worth declined (${trendStr} last month) but is still positive overall.`,
      tip: "Investigate whether the decline was a one-time expense or a recurring pattern.",
    };
  }
  return {
    id: "net_worth_trend",
    label: "Net Worth Trend",
    score: 0,
    status: `Net worth declined (${trendStr}) and remains negative.`,
    tip: "Focus on reducing debt and increasing income to reverse this trend.",
  };
}
